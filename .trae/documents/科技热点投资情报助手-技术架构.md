...
首版以高完成度前端 MVP 为目标，使用本地 Mock 数据跑通完整产品体验，同时在代码结构和数据模型上预留 Supabase、AI 摘要和数据采集能力。
...
- 数据库：Supabase Postgres，首版提供 schema 和类型定义，前端先使用 Mock 数据。
- 部署：GitHub 托管代码，Vercel 自动部署前端。
...
## 4. API 定义
首版不启用真实后端 API，使用前端 Mock 数据。为了后续接入 Supabase 和服务端 AI，先定义核心类型。
